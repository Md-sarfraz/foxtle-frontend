...
const Navbar = () => {
  ...
  const [isServiceOpen, setIsServiceOpen] = useState(false); // Dropdown toggle (desktop)
  const [isMobileServiceOpen, setIsMobileServiceOpen] = useState(false); // Dropdown toggle (mobile)
      ...
      {isOpen && (
        <ul className="absolute top-full left-0 w-full bg-white/10 backdrop-blur-md rounded-b-xl shadow-lg flex flex-col items-center gap-4 mt-2 py-4 text-gray-200 font-medium text-sm z-40">
          <li className="relative w-full group">
            <Link to="/about" onClick={() => setIsOpen(false)} className="block w-full text-center py-2">
              About Us
            </Link>
          </li>
    ...
    <li>
      ...
      <div className="hidden lg:flex items-center justify-between">
        ...
        <ul className="flex items-center gap-6 text-gray-200 font-normal text-sm">
          <li className="relative group">
            <Link to="/about" className="transition-colors duration-300 group-hover:text-white">About Us</Link>
          </li>
          ...
          <li><Link to="/products" className="group-hover:text-white">Products</Link></li>
          <li><Link to="/blogs" className="group-hover:text-white">Blogs</Link></li>
          <li><Link to="/contact" className="group-hover:text-white">Contact Us</Link></li>
        </ul>
